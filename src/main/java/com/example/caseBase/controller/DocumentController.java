...
public class DocumentController {
    ...
    public ResponseVO delete(@RequestBody DeleListRestfulVO del) {
        ...
        for(int i = 0; i < delList.size(); i++) {
            ...
            if (documentService.findByDocId(docId) != null) {
    ...
    public ResponseVO modify(@RequestBody DocVO doc) {
        ...
        if ( documentService.findByDocId(doc.getDocId()) == null) {
    ...
    public ResponseVO release(@RequestBody ReleaseListRestfulVO releaseListVO) {
        ...
        for (int i = 0; i < length; i++) {
            Doc temp = documentService.findByDocId(Long.parseLong(releaseList.get(i)));
            if (temp == null) {
                flag = true;
                break;
            }
    ...
    public ResponseVO query(@RequestBody QueryBody body){
        List<Doc> docs = documentService.queryDocs(body);
        ResponseVO responseVO = ResponseVO.buildSuccess(docs);
        return responseVO;
    }
    ...
    public ResponseVO getFileById(@PathVariable("id")String id){
        ...
        return ResponseVO.buildSuccess(documentService.findByDocId(Long.parseLong(id)));
    }
