...
public class Clawer {
    ...
    public Clawer(DocumentService documentService) {
        this.documentService = documentService;
    }

    public HashMap<String, JSONObject> sendGet(String urlStr) throws IOException, JSONException, FileNotFoundException {
    ...
    public void getDocURL(int mode) throws IOException, JSONException, ParseException, InterruptedException {
        ...
        for (int i = 1; i <= 2000; i++) {
            ...
            Date start = df.parse("2010-01-01 00:00:00");
            Date end = df.parse("2022-09-31 23:59:59");
            for (Object row : rows) {
    ...
    public void getHtml(Long docId, String subTitle, Date date) throws JSONException, IOException, InterruptedException,FileNotFoundException {
        Doc doc = new Doc();
        doc.setState(1);
        ...
        if (trs.size() > 5 && trs.size() - skip > 5) {
            ...
            if (rowspan == 2) {
                ...
                if (trs.get(skip + 1).select("td").size() == 3) {
                    doc.addLitigant(new Litigant(trs.get(skip + 1).select("td").get(2).text(), "", 1, doc));

                } else {
                    doc.addLitigant(new Litigant(trs.get(skip + 1).select("td").get(3).text(), trs.get(skip + 2).select("td").get(1).text(), 1, doc));
                }
            } else if (rowspan == 1) {
                //个人
                doc.setType(0);
                doc.addLitigant(new Litigant(trs.get(skip + 1).select("td").get(1).text(), "", 0, doc));
            } else if (rowspan == 3) {
                ...
                if (this.nullStringList.contains(trs.get(skip + 1).select("td").get(2).text())) {
                    ...
                    if (!this.nullStringList.contains(trs.get(skip + 2).select("td").get(2).text())) {
                        //单位也不为空，既有个人，也有单位
                        doc.setType(2);
                        doc.addLitigant(new Litigant(trs.get(skip + 2).select("td").get(2).text(), trs.get(skip + 3).select("td").get(1).text(), 1, doc));
                    ...
                    doc.addLitigant(new Litigant(trs.get(skip + 1).select("td").get(2).text(), "", 0, doc));
                ...
                if (trs.get(skip + 1).select("td").size() == 4) {
                    ...
                    if (!personIsNull) {
                        //个人不为空
                        doc.addLitigant(new Litigant(trs.get(skip + 1).select("td").get(3).text(), trs.get(skip + 2).select("td").get(1).text(), 0, doc));
                    }
                    if (!unitIsNull) {
                        //单位不为空
                        doc.addLitigant(new Litigant(trs.get(skip + 3).select("td").get(2).text(), trs.get(skip + 4).select("td").get(1).text(), 1, doc));
                    }
        ...
        documentService.saveOneDoc(doc);
    }
