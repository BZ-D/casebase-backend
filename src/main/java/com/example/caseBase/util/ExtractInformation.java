...
public class ExtractInformation {
    ...
    public void extract(Doc doc) {
        ...
        for (Litigant li : doc.getLitigants()) {
            ...
            List<Litigant> litigantList = litigantRepository.findAllByDocId(doc.getId());
            Litigant li_found;
            if (litigantList.size() > 0) {
                li_found = litigantList.get(0);
            } else {
                li_found = li;
                li_found.setDocId(doc.getDocId());
            }
            li_found.setProvince(punish.getProvince());
            li_found.setType(li.getUnit() == null ? "机构" : "个人");
            punish.setType(li.getType());
            li_found.setName(li.getName().length() > 200 ? li.getName().substring(0, 200) : li.getName());
            li_found = litigantRepository.save(li_found);
            // if (li_found != null) {
            //     li.setId(li_found.getId());
            // } else {
            //
            // }
            punish.setLitigant_id(li_found.getId());
            punish.setLitigantName(li_found.getName());
